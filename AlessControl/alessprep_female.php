...
$query = "select time from alesstime_female_control where id = 1";
$sql = mysql_query($query, $con);
$row = mysql_fetch_array($sql);
$queryStr = "create table if not exists aless_female_control (id int primary key auto_increment , time int, orders int ,groupnum int , category int)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_POST["category"] === "Angry") {
        ...
        $con = mysql_connect($dbServer, $dbUser, $dbPass);
        mysql_select_db($dbName, $con);
        $query = "insert into aless_female_control (time,orders,groupnum,category) values(" . $row[0] . "," . $_SESSION['pos'] . "," . $_SESSION['img'][$_SESSION['pos']] . ",1)";
        $sql = mysql_query($query, $con);
        mysql_close($con);
        $_SESSION['pos'] ++;
        ...
        $con = mysql_connect($dbServer, $dbUser, $dbPass);
        mysql_select_db($dbName, $con);
        $query = "insert into aless_female_control (time,orders,groupnum,category) values(" . $row[0] . "," . $_SESSION['pos'] . "," . $_SESSION['img'][$_SESSION['pos']] .",2)";
        $sql = mysql_query($query, $con);
        mysql_close($con);
        $_SESSION['pos'] ++;
        header('Location: http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
        ...
        $con = mysql_connect($dbServer, $dbUser, $dbPass);
        mysql_select_db($dbName, $con);
        $query = "insert into aless_female_control (time,orders,groupnum,category) values(" . $row[0] . "," . $_SESSION['pos'] . "," . $_SESSION['img'][$_SESSION['pos']] .",3)";
        $sql = mysql_query($query, $con);
        mysql_close($con);
        $_SESSION['pos'] ++;
        header('Location: http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
        ...
        $con = mysql_connect($dbServer, $dbUser, $dbPass);
        mysql_select_db($dbName, $con);
        $query = "insert into aless_female_control (time,orders,groupnum,category) values(" . $row[0] . "," . $_SESSION['pos'] . "," . $_SESSION['img'][$_SESSION['pos']] . ",4)";
        $sql = mysql_query($query, $con);
        mysql_close($con);
        $_SESSION['pos'] ++;
        header('Location: http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
...
if (empty($_SESSION['pos']) || $_SESSION['pos'] === 0) {
    $_SESSION['pos'] = 0;
        ...
        mysql_select_db($dbName, $con);
        $time = $row[0] + 1;
        $query = "update alesstime_female_control set time = " . $time . " where id = 1";
        $sql = mysql_query($query, $con);
        mysql_close($con);
}
